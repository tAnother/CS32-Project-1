...
Arena::Arena(int nRows, int nCols)
	: m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nZombies(0), m_history(History(nRows,nCols))		/// NOTICE HERE!
{
...
{
	// Position (row,col) of the arena coordinate system is represented in the array element grid[row-1][col-1]
...
bool Arena::addZombie(int r, int c)
{
	...
	if (m_nZombies == MAXZOMBIES)
		return false;
	m_zombies[m_nZombies] = new Zombie(this, r, c);		/// what is a "this pointer"?
	m_nZombies++;
	return true;
}
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
	...
	for (; k < m_nZombies; k++)
	{
		/// To see if any zombies are at the position
		if (m_zombies[k]->row() == r && m_zombies[k]->col() == c)
			break;
	}

	if (k < m_nZombies  &&  m_zombies[k]->getAttacked(dir))  // zombie dies		<------SEE HERE!!
	{
		// Record the player's position
		m_history.record(m_player->row(), m_player->col());

		delete m_zombies[k];
		// Move the zombie of the last index to fill this empty k position
...
History& Arena::history()		/// why using reference?
{
	return m_history;
}
